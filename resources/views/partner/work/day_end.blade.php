                ...
                <div class="col-12 d-lg-flex align-items-lg-stretch">
                    <div class="card radius-10">
                        <div class="card-body">
                            ...
                            <span class="text-danger">@if( isset($dayend) && $dayend && $dayend['de_no'] ) {{ $dayend['created_at'] }} 에 이미 실행했습니다.@endif</span>
                        </div>
                        <div class="card-body">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="command1" name="command1" value="Y">
                                <label class="form-check-label" for="gridCheck2">입실된 회원을 모두 퇴실 및 소등 @if(isset($dayend) && $dayend && $dayend['de_command1']=="Y") <span class="btn btn-dark btn-xs disabled">이미실행</span> @endif</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="command2" name="command2" value="Y">
                                <label class="form-check-label" for="gridCheck2">좌석 조명을 전체 소등 @if(isset($dayend) && $dayend && $dayend['de_command2']=="Y") <span class="btn btn-dark btn-xs disabled">이미실행</span> @endif</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="command3" name="command3" value="Y">
                                <label class="form-check-label" for="gridCheck2">IOT 기기 종료 @if(isset($dayend) && $dayend && $dayend['de_command3']=="Y") <span class="btn btn-dark btn-xs disabled">이미실행</span> @endif</label>
                            </div>
